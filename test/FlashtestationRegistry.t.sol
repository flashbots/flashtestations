...
contract FlashtestationRegistryTest is Test {
    ...
    function test_successful_re_registerTEEService() public {
        ...
        WorkloadId expectedWorkloadId2 = bf42MockWithDifferentWorkloadId.workloadId;
        attestationContract.setQuoteResult(mockQuote2, true, mockOutput2);

        vm.expectEmit(address(registry));
        emit FlashtestationRegistry.TEEServiceRegistered(
            expectedAddress, expectedWorkloadId2, mockQuote2, bf42MockWithDifferentWorkloadId.publicKey, true
        );
        vm.prank(expectedAddress);
        ...
        vm.assertEq(WorkloadId.unwrap(workloadId2), WorkloadId.unwrap(expectedWorkloadId2), "Workload ID mismatch");
        vm.assertEq(rawQuote2, mockQuote2, "Raw quote mismatch");
        vm.assertEq(publicKey2, bf42MockWithDifferentWorkloadId.publicKey, "Public key mismatch");
